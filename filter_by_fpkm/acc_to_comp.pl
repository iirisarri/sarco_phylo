...
# reads in output from parse_trees_cc_lf.pl, selects putative contaminants and for each pair, translates acc number (HP) to comp (trinity)
...
my $usage = "acc_to_comp.pl lf_cross_contam?.txt list/list-AC-neofor list/list-AC-leppar > out\n";
...
foreach my $num (sort keys %acc) {
    #get values (comp) for Neoc
     if (exists $acc_to_comp1{$acc{$num}[0]}) {
...
# iterate again throught the hash to retrieve lepidosiren
foreach my $numb (sort keys %acc) {
    # get values (comp) for Lepi
    if (exists $acc_to_comp2{$acc{$numb}[1]}) {
        # this will print acc-comp pairs for Neoc sequences involved in cross-contam
	#print $acc{$num}[1], "\t", $acc_to_comp2{$acc{$num}[1]}, "\n";
        # populate %comp with acc and comp for Lepi
        $comp{$numb}[2] = $acc{$numb}[1];
	$comp{$numb}[3] = $acc_to_comp2{$acc{$numb}[1]};
    }
}
